...
class AVIEstimator:
    ...
    @staticmethod
    def estimate_from_tuple(query, table_stats):
        sel = 1.0
        col_left, col_right = query
        for col in col_left.keys():
            min_val = table_stats.columns[col].min_val()
            max_val = table_stats.columns[col].max_val()
            if col in col_left.keys():
                left, right = col_left[col], col_right[col]
            else:
...
class ExpBackoffEstimator:
    ...
    def estimate(range_query, table_stats):
        ...
        for col in range_query.column_names():
            ...
            col_cnt = table_stats.columns[col].between_row_count(left + 1, right)  # (left, right) -> [left, right)
            col_sel = col_cnt / table_stats.row_count
            sels_list.append(col_sel)
        sels_list = sorted(sels_list, reverse=True)
        sel, ratio = 1.0, 1.0
        for s in sels_list:
            sel *= s * ratio
            ratio /= 2.0
